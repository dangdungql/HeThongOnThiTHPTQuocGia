...
$query="SELECT * FROM dethi WHERE Made=".$ct;
$results=mysqli_query($dbc,$query);
kt_query($results,$query);
$link=mysqli_fetch_array($results,MYSQLI_ASSOC);
$linkbaihoc=$link['link'];

$linkbaihoc = ltrim($linkbaihoc, '["');
$linkbaihoc = rtrim($linkbaihoc, '"]'); 
$arr_link=explode('","',$linkbaihoc);
for($i=0;$i<count($arr_link);$i++)
{
	$a = strpos($arr_link[$i],"images");
    $arr_link[$i] = substr($arr_link[$i],$a);
    $arr_link[$i] = str_replace("images/","",$arr_link[$i]);
}
				...
				<div class="img-precontainer-mini <?php if($is_img) echo 'original-thumb' ?>">
					<?php if($config['multiple_selection']){ ?><div class="selector">
						<label class="cont">
							...
							?>
							<input type="checkbox" <?php if($check === 1) { ?> checked <?php } ?> class="selection" name="selection[]" value="<?php echo $file;?>">
							<span class="checkmark"></span>
						</label>
					</div>
					<?php } ?>
